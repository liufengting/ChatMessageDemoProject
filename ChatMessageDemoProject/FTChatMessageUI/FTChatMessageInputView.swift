...
class FTChatMessageInputView: UIView, UITextViewDelegate{
    ...
    convenience init(frame: CGRect, otherButtons: String) {
        ...
        inputTextView = UITextView(frame: CGRectMake(FTDefaultMargin*2 + FTDefaultButtonSize, FTDefaultTextViewMargin , textViewWidth, self.bounds.height - FTDefaultTextViewMargin*2))
    ...
    func textViewDidChange(textView: UITextView) {
        if let text : NSString = textView.text as NSString {
            ...
            if (delegate != nil) {
                delegate!.FTChatMessageInputViewShouldUpdateHeight(min(max(textRect.height + FTDefaultTextViewMargin*2 + textView.textContainerInset.top + textView.textContainerInset.bottom, FTDefaultInputViewHeight), 150))
            }
        }
    }
    ...
    override func drawRect(rect: CGRect) {
        ...
        self.inputTextView.frame = CGRectMake(FTDefaultMargin*2 + FTDefaultButtonSize, FTDefaultTextViewMargin, self.textViewWidth, self.bounds.height - FTDefaultTextViewMargin*2)
    }
